...
- **GSAP** lives next to the component it animates (the fullscreen preview).
  Reused scroll/hover helpers would move to `shared/hooks/`.
- **Interactive organisms** are mounted as Astro islands: `client:visible`
  for the app shell and filterable lists, `client:load` for sliders and
  fullscreen previews. Without JS the static markup (sections, slider
  levels) still renders.
- 3D/R3F/Three, react-hook-form and zod are **removed** from the project —
  do not reintroduce those patterns.
...
emits title/description/canonical/hreflang from `resolvePageMeta`, bootstraps
