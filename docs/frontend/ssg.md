...
У приложения нет страницы `/`, все URL живут под `/:lang`. Плагин всегда
стартует обход с `/`, поэтому адаптер маппит `/` на `routeUrl = "/ru"` и
рендерит русский дом. `head.lang` считает из исходного url:
`getLangFromPath("/")` даёт `"ru"`.

Так `dist/index.html` это страница `/ru`, а остальные страницы живут под
`dist/{ru,en}/...`.
