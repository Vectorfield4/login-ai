...
/**
 * Слой новостей: чистые функции над типом `NewsItem` и разбором `id` коллекции.
 *
 * Здесь НЕТ импорта `astro:content` — намеренно. Модуль читается юнит-тестами
 * Vitest, где виртуальный модуль Astro недоступен, а логика разбора имён
 * файлов и сборки ссылок должна быть покрыта тестами. Доступ к коллекции живёт
 * отдельно, в `newsCollection.ts`, и вызывается только из frontmatter
 * `.astro`-страниц.
 */
...
export interface NewsData {
  ...
  relatedCases?: string[];
  /** Растр из frontmatter (`image()` в схеме коллекции). */
...
export interface NewsItem {
  /** Слаг без языка — идентификатор статьи в пределах локали. */
  slug: string;
  lang: NewsLang;
  title: string;
  description: string;
  ...
  draft: boolean;
  tags: string[];
  /** Растр для og:image; страница передаёт его в BaseLayout. */
  ogImage?: ImageMetadata;
  /** Локализованная ссылка на статью (с префиксом `/ru` или `/en`). */
  href: string;
  /** Чистый путь статьи — для канонического URL, крошек и schema.org. */
  path: string;
  /** Дата публикации строкой в локали статьи. */
  publishedLabel: string;
  /** ISO-дата для `<time datetime>` и schema.org. */
  publishedIso: string;
  /** Ссылки на коммерческие сущности — совместимы с `relevants`. */
  relevants: EntityRef[];
}
